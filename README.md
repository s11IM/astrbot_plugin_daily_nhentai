...
这个插件会每日（通过命令）获取 nhentai 的中文最新本子，并使用本地 YOLO NSFW 模型进行质量筛选。

## 功能特性
1. **自动爬取**: 绕过 Cf 获取每日列表，并自动过滤少于 35 页的短篇本子。
...
/nh today
```
Bot 将自动爬取 nhentai 中文列表，进行下载和 AI 分析，最后发送一张包含今日最佳 10 本的评分卡片。
